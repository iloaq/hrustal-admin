...
export async function POST(request: NextRequest) {
  try {
    ...
    const createdDriverVehicles = [];
    for (const assignment of driverVehicleAssignments) {
      try {
        const result = await prisma.driverVehicle.create({
          data: {
            driver_id: BigInt(assignment.driverId),
            vehicle_id: BigInt(assignment.vehicleId),
            assigned_at: new Date(),
            is_primary: Math.random() > 0.5 // Случайно назначаем основного водителя
          }
        });
        createdDriverVehicles.push(result);
    ...
    const createdVehicleDistricts = [];
    for (const assignment of vehicleDistrictAssignments) {
      try {
        const result = await prisma.vehicleDistrict.create({
          data: {
            vehicle_id: BigInt(assignment.vehicleId),
            district_id: BigInt(assignment.districtId),
            assigned_at: new Date()
          }
        });
        createdVehicleDistricts.push(result);
    ...
    const createdDriverDistricts = [];
    for (const assignment of driverDistrictAssignments) {
      try {
        const result = await prisma.driverDistrict.create({
          data: {
            driver_id: BigInt(assignment.driverId),
            district_id: BigInt(assignment.districtId),
            assigned_at: new Date()
        ...
        createdDriverDistricts.push(result);
    ...
    return NextResponse.json({
      success: true,
      message: 'Тестовые привязки восстановлены!',
      stats: {
        driverVehicles: finalStats[0],
        vehicleDistricts: finalStats[1],
        driverDistricts: finalStats[2]
      },
      created: {
        driverVehicles: createdDriverVehicles.length,
        vehicleDistricts: createdVehicleDistricts.length,
        driverDistricts: createdDriverDistricts.length
      }
    });
